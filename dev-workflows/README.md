...
- **Source-code is the truth** (`_shared/source-truth.md`, v1.7.0+): every sub-agent that writes or reviews user-visible docs (`doc-planner`, `doc-reviewer`, `code-scanner`, `epic-reviewer`) verifies enum/option lists, UI labels, defaults, counts, and mode names against the actual source code in the user's repos — not the Jira ticket description. Documented values that don't appear in source are BLOCKERS, not CONCERNS, because customers will use the wrong info.
