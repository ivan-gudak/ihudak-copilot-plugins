...
allowed-tools: view, grep, glob, task, ask_user, sql
---
...
if in_plan_creation:
    ...
    ask_user(
        ...
        choices=[
            "Quick (2-3 min) — Plans, PRs, localized changes",
            "Deep (5-10 min) — Before implementation, global scope"
        ]
    )
```
...
### Step 3: Present Risks via ask_user
...
How would you like to proceed?""",
    choices=[
        ...
        "Add mitigations to plan (Recommended)",
        "Research mitigation options",
...
for tiger in high_severity_tigers:
    ...
    task(
        description="Research best practices externally",
        prompt=f"""
...
# Synthesize options
# Present via ask_user with 2-4 mitigation options
```
...
ask_user(
    ...
    choices=[format_risk_option(r) for r in all_risks[:4]]
)
...
/premortem deep plans/YYYY-MM-DD-feature.md
