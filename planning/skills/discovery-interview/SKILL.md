...
allowed-tools: view, glob, grep, bash, web_fetch, ask_user, task, create
---
...
1. **Ask 2-4 questions** using ask_user
...
1. Spawn a task agent or use web_fetch/bash
...
2. **Generate the spec** to `specs/YYYY-MM-DD-<name>.md` relative to the project root; if that directory doesn't exist, check `/specs/` (externally mounted docs volume); if neither exists, create `specs/` in the current working directory:
...
You: [ask_user: "Who specifically will use this? What's their cooking level?"]
User: "Home cooks, beginners to intermediate"

You: [ask_user: "Walk me through: someone opens your app for the first time. What do they see? What do they do first?"]
...
You: [Detected uncertainty in UX]
    ...
    [ask_user: 'What's the ONE thing a user should accomplish in their first 2 minutes?'
    Options:
...
You: [Spawn research task or web_fetch]
...
You: "I researched successful recipe apps. Here's what I found:
    ...
    Given this, let's refine: [ask_user with informed options]"
...
```
ask_user(
  question: "Spec created at specs/YYYY-MM-DD-<name>.md (or /specs/ if externally mounted). How would you like to proceed?",
  options: [
